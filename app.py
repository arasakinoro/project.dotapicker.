...
# Глобальные словари для ID героев
name_to_id = {}
id_to_name = {}

# Загрузка героев с OpenDota (один раз при запуске)
@st.cache_data
def load_heroes():
    ...
    return f"Загружено {len(heroes)} героев"

# Получение matchup (винрейт против всех)
@st.cache_data
def get_matchups_from_opendota(hero_id):
    ...
    try:
        ...
        time.sleep(0.5)  # пауза внутри кэша
        return matchups
    except:
        return {}
...
def recommend_heroes(my_role, enemies_names, top_k=7, mode='average'):
    ...
    if valid_enemies == 0:
        return ["Нет данных по врагам"] * top_k
...
st.set_page_config(page_title="Dota 2 Рекомендатор", layout="centered")

st.title("Dota 2 Рекомендатор героев (патч 7.40)")
st.markdown("Укажи свою роль и врагов — получи топ-7 пиков для победы!")

# Загрузка героев (при первом запуске)
load_heroes()
...
st.markdown("---")
st.caption("Данные: OpenDota API | Разработано для школьного проекта")
